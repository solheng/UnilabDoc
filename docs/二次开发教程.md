...
| 19 | add_out_line | 流程输出连线 |model |add_subprocess/add_graph_model |
    ...
    # 1. 通过流程入口对象 创建流程图模型
    model = sln.get_calcmodel_cli().add_graph_model(name="demo_process")
    # 2. 设置流程的全局输入输出变量
    model.add_input_variable(name="file_content",datatype_id=DataType.String, is_to_global=True)
    ......(省略流程更多全局的输入输出变量)
    model.add_output_variable(name="logs", datatype_id=DataType.String, is_from_global=True)

    # 3. 增加函数节点log_file_check
    model.add_func_node(name="log_file_check", function="""
    import os
    ...
    # 5. 流程对象增加exe执行节点并配置相关参数
    model.add_exe_node(name="exe_node", is_private_cwd=False, description="型线导入程序",
    ...
    # 6. 流程对象增加func2函数节点
    node = model.add_func_node(name="func2", function='''
    from dq import unit
